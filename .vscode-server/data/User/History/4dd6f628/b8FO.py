...
class Google:
    ...
    def search(self, input_text):
        ...
        ## 追加
        self.url_cur = self.driver.current_url
    
    def get_url(self):
        count = 0
        url = []
        for e in self.driver.find_elements(By.XPATH, "//a/h3"):
            self.elem_a = e.find_element(By.XPATH, "..")
    ...
    def get_other_keyword(self):
        other_keywords = []
        response = requests.get(self.url_cur)
        soup = BeautifulSoup(response.content, "html.parser")
        # ここby.xpathでfindelementするように書き換える
        others = soup.find_all(class_ = "BNeawe s3v9rd AP7Wnd lRVwie") ## よくわからんクラス名で取れても意味ないので、XPATHで取れないかやってみる
        logger.debug(others)
        # words = [i.text for i in others]
        for i in others:
            other_keywords.append(i.text)
