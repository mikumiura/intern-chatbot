...
class DataBase:
    ...
    def insert_review(self, star_rate, review_comment):
        try:
            with self.conn.cursor() as cursor:
                sql = "insert into `review` (star_rate, review_comment) values (%s, %s)"
    ...
    def select_avg_starrate(self):
        ...
        try:
            with self.conn.cursor() as cursor:
                sql = "select round(avg(star_rate), 1) from `review`"
                cursor.execute(sql)
    ...
    # チャットボットの高評価コメントを取得
    def select_highlyrated_comment(self, empty):
        highly_rated_comment = None
        try:
            with self.conn.cursor() as cursor:
                sql = "select review_comment from `review` where review_comment != %s order by star_rate desc limit 3"
                cursor.execute(sql, empty)
                highly_rated_comment = cursor.fetchone()
        except:
            logger.exception("exception happened.")
        return highly_rated_comment

    ## 変数の初期化全部Noneでやっちゃってるけど平気か？？