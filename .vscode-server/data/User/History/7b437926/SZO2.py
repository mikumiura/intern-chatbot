...
class DataBase:
    ...
    def insert_review(self, star_rate, review_message):
        try:
            with self.conn.cursor() as cursor:
                sql = "insert into `review` (star_rate, review_message) values (%s, %s)"
                cursor.execute(sql, (star_rate, review_message))
            self.conn.commit()
        except:
            logger.exception("exception happened.")

    # チャットボットの評価（平均星数、高評価コメント）を取得