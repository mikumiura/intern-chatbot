...
class DataBase:
    ...
    # チャットボットの評価をDBにインサート
    def insert_review(self, star_rate, user_name, review_comment):
        try:
            with self.conn.cursor() as cursor:
                sql1 = "insert into `reviews` (star_rate, user_name, review_comment) values (%s, %s, %s)"
                cursor.execute(sql1, (star_rate, user_name, review_comment))
                sql2 = "update `reviews` set user_name = %s where user_name = NULL"
                cursor.execute(sql2, '名無し')
            self.conn.commit()
        except:
            logger.exception("exception happened.")
    ...
    # 星評価数に紐づくコメントとニックネームを3件取得
    def select_review_comment(self, num):
        comments = None
        try:
            with self.conn.cursor() as cursor:
                sql = "select distinct user_name, review_comment from `reviews` where star_rate = %s order by id desc limit 3"
