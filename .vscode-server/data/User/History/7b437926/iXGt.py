...
class DataBase:
    ...
    # DBにあるAND検索のキーワードのうち、最初に溜まった1件を取得　※fetchall()だとAND検索のキーワード全件返るが、post_to_line()でforが使えない
    ...
    # チャットボットへの評価をDBにインサート
    def insert_review(self, star_rate, review_comment):
        try:
            with self.conn.cursor() as cursor:
                sql = "insert into `reviews` (star_rate, review_comment) values (%s, %s)"
                cursor.execute(sql, (star_rate, review_comment))
            self.conn.commit()
        except:
            logger.exception("exception happened.")

    # チャットボットの平均星評価を取得
    def select_avg_starrate(self):
        avg_star_rate = None
        try:
            with self.conn.cursor() as cursor:
                sql = "select round(avg(star_rate), 1) from `reviews`"
                cursor.execute(sql)
                avg_star_rate = cursor.fetchone()
        except:
            logger.exception("exception happened.")
        return avg_star_rate["round(avg(star_rate), 1)"]

    # 星評価数に紐づくコメントを3件取得
    def select_review_comment(self, num):
        comment = None
        comment_list = []
        try:
            with self.conn.cursor() as cursor:
                sql = "select distinct review_comment from `reviews` where star_rate = %s order by review_comment desc limit 3"
                cursor.execute(sql, num)
                comment = cursor.fetchall()
                for c in comment:
                    comment_list.append(c["review_comment"])
        except:
            logger.exception("exception happened.")
        return comment_list

    # # チャットボットの高評価コメントを取得
    # def select_highlyrated_comment(self):
    #     comment = None
    #     highly_rated_comment = []
    #     try:
    #         with self.conn.cursor() as cursor:
    #             sql = "select review_comment from `reviews` order by star_rate desc limit 3"
    #             cursor.execute(sql)
    #             comment = cursor.fetchall()
    #             for c in comment:
    #                 highly_rated_comment.append(c["review_comment"])
    #     except:
    #         logger.exception("exception happened.")
    #     return highly_rated_comment
